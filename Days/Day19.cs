...
public class Day19 : AdventOfCode<long, Day19Data>
{
    ...
    {
      var tps = new List<TheoreticalPart>();
      tps = MatchTheoreticalPart(world.Workflows, "in", new TheoreticalPart(new Part(1,1,1,1), new Part(4000, 4000, 4000, 4000))).ToList();
        
      var oldTps = new List<TheoreticalPart>();
      while(tps.Count != oldTps.Count)
      {
        oldTps = tps;
        tps = tps.Pairs().SelectMany(it => SplitOverlaps(it.First, it.Second)).ToList();
      }
    ...
    public IEnumerable<TheoreticalPart> NarrowAndApply(Workflows workflows, Rule rule, TheoreticalPart part)
    {
      ...
      if (p != null)
      {
        if (rule.Destination == "A") yield return part;
